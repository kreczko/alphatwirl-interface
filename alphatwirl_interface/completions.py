...
def complete(df_configs, completer):
    ...
    reader_collector_pairs = [build_counter_collector_pair(
        config) for config in df_configs]
...
def build_counter_collector_pair(df_cfg):
    ...
    nextKeyComposer = alphatwirl.summary.NextKeyComposer(
        df_cfg['binnings']) if df_cfg['binnings'] is not None else None
    ...
    resultsCombinationMethod = alphatwirl.collector.ToDataFrame(
        summaryColumnNames=df_cfg['keyOutColumnNames'] +
        df_cfg['valOutColumnNames']
    )
    deliveryMethod = alphatwirl.collector.WritePandasDataFrameToFile(
        df_cfg['outFilePath'])
